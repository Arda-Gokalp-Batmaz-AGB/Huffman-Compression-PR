...
#import GrayLevelCompression.filters as filters
import csv
import os
...
def DecodeData(root_node,pivotvalue,rowc,columnc,bit_list,process_method_of_image):
  #bit_list = readBinaryFile("compressedversion.txt")
  temp_node = root_node
  result = []
  #rowc=650 #simdilik kalsins
  counter = 0
  temp=[]
  ...
  if(process_method_of_image=="Difference"):
    result = RecoverDifferencedArray(result, pivotvalue)  # ŞİMDİLİK 21 KALSIN SONRA PİVOT ALMA DÜZENLENCEK 167 13 21
...
def GetBitArray(encoded_string):
  ...
  for i in range(0, len(encoded_string), 8):
    upper = i+8
   # if(upper >=len(encoded_string)):
     #  upper = len(encoded_string)+1
...
def Read_Image(file : str,process_method_of_image):
  list = []
  img = Image.open(file)
  img = img.convert('L')#filters.color2gray(img) # convert to gray
...
def ReadDictionaryFile(path):
  with open(path) as csv_file:
    ...
    keylength = eval(csvlist[1][1])#don't return
    gray_map = dict()
...
def RunGrayCompression(filename,process_method_of_image):
  ...
  # binarycodedstring = readBinaryFile(f"{path}{name_without_extension}compressedversion.bin")
  # result = DecodeData(root_node, read_data.shape[1], read_data.shape[0],binarycodedstring,process_method_of_image)
  # im = filters.np2PIL(result)
  # #im.show()
  # im.save(f"{path}{name_without_extension}decompressedversion.png")

  #im = DecodeCompressedFile(f"{path}{name_without_extension}", process_method_of_image,f'{name_without_extension}')
  im = None
...
if __name__ == "__main__":
  RunGrayCompression("cam.PNG","Difference")
