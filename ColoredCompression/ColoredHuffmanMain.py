...
#import ColoredCompression.filters as filters
...
def EncodeData(current_node : Node, code : str,coded_dict):
  ...
  if current_node.character != '-':
   # print(str(current_node.character) + ':' + code)
    coded_dict[current_node] = code
    code = ""
  EncodeData(current_node.left,code + '1',coded_dict )#0
  EncodeData(current_node.right, code + '0',coded_dict )#1

  #temp = coded_dict.copy()
  #coded_dict = dict()
  return coded_dict
...
def DecodeData(root_node,pivot,coded_string,rowc,columnc,process_method_of_image):
  ...
  for tempbit in bit_list:
      ...
      if (temp_node.character != '-'):
       # result.append(temp_node.character)
  ...
  result = numpy.array(result, dtype=numpy.int8)#original without , dtype=numpy.int8
...
def Read_Image(file : str,process_method_of_image):
  list = []
  img = Image.open(file)
  #img = filters.color2gray(img) # convert to gray
  img = img.convert('RGB')
  arr = numpy.array(img)#filters.PIL2np(img)
...
def CalculateDifferencedArray(char_list):
  #char_list = numpy.array([[21, 21, 21, 95, 169, 243, 243, 243],[21, 21, 21, 95, 169, 243, 243, 243],[21, 21, 21, 95, 169, 243, 243, 243],[21, 21, 21, 95, 169, 243, 243, 243]])
...
# def CreateZipFile(mainpath):
#   zipObj = ZipFile(f'{mainpath}BinaryandDictionary.zip', 'w')
#   # Add multiple files to the zip
#   zipObj.write('sample_file.csv',f'{mainpath}BinaryandDictionary.zip')
#   zipObj.close()
...
def StoreDictionaryandOthers(red_character_map,green_character_map,blue_character_map,mainpath,redlen,greenlen,bluelen,redex,greenex,blueex,rowc,columc,encoded_string,name,process_method_of_image,extension):
  ...
  os.remove(f"{mainpath}dictionary.csv") #SİLME İŞLEMİ EN SON EKLENECEK
  os.remove(f"{mainpath}compressedversion.bin") #SİLME İŞLEMİ EN SON EKLENECEK
...
def DecodeCompressedFile(mainpath,process_method_of_image,name,fromexternalzip=False):
  ...
  im = Image.fromarray(numpy.uint8(result))#filters.np2PIL(result)
  # im.show()
...
def WriteBinaryFile(mainpath,encoded_string):
  f = open(f"{mainpath}compressedversion.bin", "wb")#  f = open("datafiles/compressedversion.txt", "w")
...
def RunColoredCompression(filename,process_method_of_image):
  ...
  #WriteBinaryFile(f"{path}{name_without_extension}",encoded_string)
  # f = open(f"{path}{name_without_extension}compressedversion.bin", "wb")#  f = open("datafiles/compressedversion.txt", "w")
  # b = GetBitArray(encoded_string)
  # f.write((b))
  # f.close()
  ...
  #
  # red,green,blue = readBinaryFile(f"{path}{name_without_extension}compressedversion.bin",len(red_string),len(green_string),len(blue_string))
  #
  #
  # red_component = DecodeData(red_root_node, red_pivot,red[0:len(red)-red_extras],read_data.shape[1],read_data.shape[0],process_method_of_image)
  # green_component = DecodeData(green_root_node, green_pivot, green[0:len(green)-green_extras], read_data.shape[1], read_data.shape[0],process_method_of_image)
  # blue_component = DecodeData(blue_root_node, blue_pivot,blue[0:len(blue)-blue_extras],read_data.shape[1],read_data.shape[0],process_method_of_image)
  #
  # result = rgb.MergeRGB(red_component,green_component,blue_component)
  # im = filters.np2PIL(result)
  # #im.show()
  # im.save(f"{path}{name_without_extension}decompressedversion.png")

  #im = DecodeCompressedFile(f"{path}{name_without_extension}",process_method_of_image,f'{name_without_extension}',extension)
  im = None
  #os.remove(f"{path}{name_without_extension}decompressedversion.png")
...
if __name__ == "__main__":
  RunColoredCompression("datafiles/apple.png")
  #DecodeCompressedFile('C:/Users/arda/PycharmProjects/FirstProject/venv/Project/datafiles/pix_',"Difference","pix_")
