...
def upload_file():
    f_types = [('PNG Files','*.png'),('BMP Files','*.bmp')]   # type of files to select ('Jpg Files', '*.jpg')
    global filename
    global lastfilename
    ...
    #input_size_label.config(text = f"Total Size = {round(os.stat(filename).st_size / 1000)}")
    input_image_size_label.place(x=x/3, y=y*6)
...
def UploadZip():
    ...
    zip_name_label.place(x=width / 2.8, y=(height/1.75)-30)
...
def TextCompressThread():
    ...
    # switchButtonState(compress_image_button)
    switchButtonState(decompress_upload_button)
    switchButtonState(text_upload_button)
    # switchButtonState(decompress_data_button)
    ...
    switchButtonState(level3decompress)
    switchButtonState(level4decompress)
    switchButtonState(level5decompress)
...
def TextDeCompressThread():
    ...
    # if ('differencevalue_label' in globals()):
    differencevalue_label.config(text="")
...
def TextDeCompression(animation_label):
    ...
    f = open(first_text_path, "r")
    first_text = f.read()
    first_text = first_text.rstrip()

    differencevalue = 0
    for i in range(0,len(decompressed_text)):
        differencevalue = ord(decompressed_text[i])-ord(first_text[i])
    ...
    differencevalue_label.place(x=x * 7, y=y * 6)
...
def CreateLabels(width,height):
   pass# input_size_label = tk.Label(my_w,font=font)
   # l1 = tk.Label(my_w, text='Upload Files & display', width=30, font=font)
   # l1.grid(row=1, column=1, columnspan=4)

    #input_size_label = f"Total Size = {os.stat('somefile.ext').st_size}"
def CreateButtons(width,height):
    # upload_button = Button(my_w, text='Upload Image', font=font, width=27, command=lambda: upload_file())
    # compress_image_button = Button(my_w, text='Compress Image', font=font, width=27,command=lambda: CompressionThread())
    upload_button.place(x=width / 2.55, y=height / 12)#1.5#=width / 10
    #compress_image_button.place(x=width / 2.8, y=height/1.75)#1.2#3

    #color_label.place(x=(width/3)+70, y=(height/1.2)-400)
    #radio_button_colored.place(x=(width / 3)+60, y=(height/1.2)-370)
    #radio_button_gray.place(x=(width / 3)+60, y=(height / 1.2)-340)

    #method_label.place(x=(width / 3)+70, y=(height / 1.2) - 280)
    #radio_button_difference.place(x=(width / 3)+60, y=(height/1.2) - 250)
    #radio_button_standart.place(x=(width / 3)+60, y=(height / 1.2) - 220)

    #decompress_data_button.place(x=width / 20, y=height / 1.2)
    decompress_upload_button.place(x=width / 2.55, y=(height/1.6)-35)# decompress_upload_button.place(x=width / 12, y=height / 1.3)

    text_upload_button.place(x=width / 2.55, y=(height/8)+10)
    #choose_method_button
    #
    #
# def CreateFrames(width,height):
#     frame = Image.open("GuiImages/frame.png")
#     frame= frame.resize((200,200))
#     frame = frame.convert(mode="RGBA")
#     frame =ImageTk.PhotoImage(frame)
#     frame_1_label =Label(my_w)
#
#     x = width / 10
#     y = height / 12
#     frame_1_label.place(x=x, y=y)
#     frame_1_label.image = frame
#     frame_1_label['image']=frame
#     pass
...
def CompressionThread(color_of_compression,process_method_of_image):
    ...
    #if('decompressed_text_box' in globals()):
   # decompressed_text_box.delete('1.0', END)
...
def DeCompression(animation_label):
    ...
    # differencevalue_label = tk.Label(my_w,text= f"Difference = {differencevalue}", justify=LEFT,anchor="w",width=30, font=font)
    # differencevalue_label.place(x=width / 1.55, y=y)
   ...
   # differencevalue_label = tk.Label(my_w,text= f"Difference = {differencevalue}", justify=LEFT,anchor="w",width=30, font=font)
    ...
    # x = width / 10
    # y = height / 12
    # compressed_img_label.place(x=x*6.5, y=y*1.7)
    # compressed_img_label.image = img
    # compressed_img_label['image']=img #
    # input_size_label = tk.Label(my_w, text=f"Total Size = {round(os.stat(compressed_name).st_size / 1000)} KiloBytes", width=30, font=font)
    # input_size_label.place(x=x*6.5, y=y*6)
...
def SelectedMethod(preference):
    ...
    if(preference=="Difference"):
        ...
        process_method_of_image="Standart"
    print(f"Selected Color: {preference}")
...
def Compression(animation_label,color_of_compression,process_method_of_image):
   ...
   # differencevalue = ComputeDifference(compressed_img,filename)
   #  img = compressed_img
   #  img=img.resize((200,200))
   #  img=ImageTk.PhotoImage(img)
   #  compressed_img_label =Label(my_w)
...
def ShowCalculatedValues(values):#,input_size_label
    x = width / 15
    y = height / 1.6
    avg_label = tk.Label(my_w,text= f"Average Length = {values[0]:.5f}",width=30, justify=LEFT,anchor="w",font=font,background='bisque')
    entrophy_label = tk.Label(my_w,text= f"Entrophy(H) = {values[1]:.5f}",width=30, justify=LEFT,anchor="w", font=font,background='bisque')
    ratio_label = tk.Label(my_w,text= f"Compression Rate = {values[2]:.5f}",width=30, justify=LEFT,anchor="w", font=font,background='bisque')
    ...
    avg_label.place(x=width / 1.55,y=y-260)#7   2.6
    entrophy_label.place(x=width / 1.55, y=y-230)#20-40-60-80
    ratio_label.place(x=width / 1.55, y=y-200)
    before_comp_lable.place(x=width / 1.55, y=y-170)
    after_comp_lable.place(x=width / 1.55, y=y-140)
    #differencevalue_label.place(x=width / 1.55, y=y + 125)
    # avg_label.place(x=width / 1.55,y=y+25)#7   2.6
    # entrophy_label.place(x=width / 1.55, y=y+45)#20-40-60-80
    # ratio_label.place(x=width / 1.55, y=y+65)
    # before_comp_lable.place(x=width / 1.55, y=y+85)
    # after_comp_lable.place(x=width / 1.55, y=y+105)
    ClearQueue(avg_label,entrophy_label,ratio_label,before_comp_lable,after_comp_lable)#differencevalue_label#,compressed_img_label,,input_size_label

def ToggleAnimation(image_label,x=0,y=0):
   # image_label = tk.Label(my_w,width=30)
   ...
   if(not hasattr(image_label,'frames')):
       ...
       # x = width / 10
       # y = height / 12
       # image_label.place(x=x * 6.5+50, y=y * 1.7+50)
       #image_label.place(x=width / 2.4, y=height/1.9)
   else:
       image_label.unload()

   return  image_label

def ClearQueue(avg_label,entrophy_label,ratio_label,before_comp_lable,after_comp_lable):
   ...
   # global input_size_cleared
  #  global differencevalue_label_cleared
    ...
    #input_size_cleared=input_size_label
    #image_cleared=compressed_image_label
   # differencevalue_label_cleared=differencevalue_label
def ClearImage():
...
#    input_size_cleared.config(text="")
    #differencevalue_label_cleared.config(text="")
  #  image_cleared.image = None
  #  image_cleared['image']=None #
...
def InitializeComponents():
   ...
   global radio_button_standart
   # global color_label
   # global method_label
   global differencevalue_label
   differencevalue_label = tk.Label(my_w,  justify=LEFT, anchor="w", width=30,
                                    font=('times', 12, 'bold'),background='bisque')
  # global differencevalue_label
  #  color_label=tk.Label(my_w, text=f"Choose Color Level",justify=LEFT,anchor="w", width=30, font=font)
  #  method_label=tk.Label(my_w, text=f"Choose Method",justify=LEFT,anchor="w", width=30, font=font)
   ...
   radio_button_standart = tkinter.Radiobutton(text='Standart',font=font, variable=2, value=2,
                                           command=lambda: SelectedMethod("Standart"))
    ...
    if(outputcolor=="Colored" and inputcolor=="Colored"):
        for i in range(len(input_image_array)):
            for j in range(len(input_image_array[i])):
                for k in range(0,3):
                    diff = diff + pow((input_image_array[i][j][k]-output_image_array[i][j][k]),2)
        return diff
    if(outputcolor=="Gray" and inputcolor=="Gray"):
        for i in range(len(input_image_array)):
            for j in range(len(input_image_array[i])):
                    diff = diff + pow((input_image_array[i][j]-output_image_array[i][j]),2)
        return diff
    return -1
...
def CompandDecompButtons():
    ...
    global level3decompress
    global level4decompress
    global level5decompress

    x = width / 10
    y = height / 12
    level1compress = tk.Button(my_w, text='Level 1 Compress', bg='lightblue', font=font, width=20)
    level1compress.place(x=width / 2.55, y=(height / 6)+20)  # 1.5#=width / 10
    level1compress['command'] = lambda: TextCompressThread()

    level2compress = tk.Button(my_w, text='Level 2 Compress', bg='lightblue',font=font, width=20)
    level2compress.place(x=width / 2.55, y=(height / 6)+55)  # 1.5#=width / 10
    level2compress['command'] = lambda: CompressionThread("Gray","Standart")

    level3compress = tk.Button(my_w, text='Level 3 Compress', bg='lightblue', font=font,width=20)
    level3compress.place(x=width / 2.55, y=(height / 6)+90)  # 1.5#=width / 10
    level3compress['command'] = lambda: CompressionThread("Gray","Difference")

    level4compress = tk.Button(my_w, text='Level 4 Compress', bg='lightblue', font=font,width=20)
    level4compress.place(x=width / 2.55, y=(height / 6)+125)  # 1.5#=width / 10
    level4compress['command'] = lambda: CompressionThread("Colored","Standart")

    level5compress = tk.Button(my_w, text='Level 5 Compress', bg='lightblue', font=font,width=20)
    level5compress.place(x=width / 2.55, y=(height / 6)+160)  # 1.5#=width / 10
    level5compress['command'] = lambda: CompressionThread("Colored","Difference")


    level1decompress = tk.Button(my_w, text='Level 1 Decompress', bg='lightgreen', font=font,width=20)
    level1decompress.place(x=width / 2.55, y=(height / 1.6))  # 1.5#=width / 10
    level1decompress['command'] = lambda: TextDeCompressThread()

    level2decompress = tk.Button(my_w, text='Level 2 Decompress', bg='lightgreen', font=font,width=20)
    level2decompress.place(x=width / 2.55, y=(height / 1.6)+35)  # 1.5#=width / 10
    level2decompress['command'] = lambda: DeCompressionThread()

    level3decompress = tk.Button(my_w, text='Level 3 Decompress', bg='lightgreen', font=font,width=20)
    level3decompress.place(x=width / 2.55, y=(height / 1.6)+70)  # 1.5#=width / 10
    level3decompress['command'] = lambda: DeCompressionThread()

    level4decompress = tk.Button(my_w, text='Level 4 Decompress', bg='lightgreen', font=font,width=20)
    level4decompress.place(x=width / 2.55, y=(height / 1.6)+105)  # 1.5#=width / 10
    level4decompress['command'] = lambda: DeCompressionThread()

    level5decompress = tk.Button(my_w, text='Level 5 Decompress', bg='lightgreen', font=font,width=20)
    level5decompress.place(x=width / 2.55, y=(height / 1.6)+140)  # 1.5#=width / 10
    level5decompress['command'] = lambda: DeCompressionThread()
