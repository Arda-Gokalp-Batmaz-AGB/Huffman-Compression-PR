...
def DecodeData(mainpath,name,fromexternalzip=False):
  ...
  f = open(f"{mainpath}decodedText.txt", "w")
  f.write(f"{result}")
  os.remove(f"{mainpath}Map.csv")
  os.remove(f"{mainpath}compressedbinary.bin")
...
def RunTextCompression(filename):
  ...
  #DecodeData(f"{path}{name_without_extension}",f'{name_without_extension}')

  return f"{path}{name_without_extension}decompressedversion{extension}",values
if __name__ == "__main__":
  pass
  RunTextCompression("C:/Users/arda/PycharmProjects/FirstProject/venv/Project/datafiles/level1yedek/text2.txt")
  #DecodeData(f"C:/Users/arda/PycharmProjects/FirstProject/venv/Project/datafiles/level1yedek/text_", f'text_')

  # read_data = Read_File("sample.txt")
  #
  # character_map = FillMap(read_data)
  # total_count = len(read_data)
  #
  # root_node = BuildHuffmanTree()
  #
  # code_map = EncodeData(root_node,"")
  #
  # SaveEncodedData(read_data,code_map)
  #
  # DecodeData(root_node)

## BİR ŞEKİLDE GEREKSİZLERİ BİRLEŞTİR, CLASS AYRIMI YAP, NODE CLASSINI KULLANARAK GEREKSİZ YERLERİ YOK ET NODE CLASINI AYIR
## CALCULATİONLARI BİR ŞEKİLDE DÜZENLE KOD TEKRARINI AZALT TRİPLET YPA GEREKİRSE
...
#def TotalBenefit(first_data,character_map,coded_map):
 # before_compression = len(first_data) * 8
  #after_compression = 0
  #for key in coded_map:
   # after_compression = after_compression + (len(coded_map[key]) * character_map[key])
  #print(f"Before Operation = {before_compression}")
  #print(f"After Operation = {after_compression}")
  #print(f"Total Benefit = {before_compression - after_compression}")
